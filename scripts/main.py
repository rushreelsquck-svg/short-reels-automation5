"""
main.py
Runs the whole daily history pipeline end to end. Each stage prints progress
...
from generate_history import generate_history_video
...
WORKDIR = Path("/tmp/history_run")


def run():
    print("[1/5] Writing today's history video (Claude)...")
    video = generate_history_video()
    print(f"      -> {video['title']}  ({len(video['facts'])} facts)")
    ...
    for i, fact in enumerate(video["facts"]):
        ...
        generate_voiceover(fact["narration"], audio_path)
        scenes.append({
            "audio_path": audio_path,
            "visual_query": fact["visual_query"],
            "caption_text": fact["narration"],
            "number": i + 1,
        })
